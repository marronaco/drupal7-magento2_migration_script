	  ...
	  $orderData_address=[		
		...
		// Creating Array of Transaction ID
		 ...
		 if(count($result_order_transaction, 1) >= 1) { 
			 ...
			 $trans_id = array('a' => 123456789);
			 $trans_mc_gross = array('a' => 0);
			 }	
			 //Converting in Base64 for Mofluid API			 			
			...
			file_put_contents($log_file_order, $value_uc_order['order_id'].'--->'.$value_uc_order['uid'].'--->'.'guest'."\n", FILE_APPEND);					
				try{
					...
					$curlConfig = array(
						...
						CURLOPT_POSTFIELDS     => array('store'=>1, 'currency'=>'USD', 'service'=>'placeorder', 'customerid'=>0, 'products'=>$decodedtext_product, 'address'=>$decodedtext_address, 'is_create_quote'=>1, 'shipmethod'=>'freeshipping_freeshipping', 'paymentmethod'=>'cashondelivery', 'couponCode'=>'null','transactionid' =>$encoded_trans_id, 'order_status' =>$value_uc_order['order_status'], 'trans_gross'=>$encoded_trans_mc_gross)
					);
				...
				try{
					...
					$curlConfig = array(
						...
						CURLOPT_POST           => true,
						CURLOPT_RETURNTRANSFER => true,
						CURLOPT_POSTFIELDS     => array('store'=>1, 'currency'=>'USD', 'service'=>'placeorder', 'customerid'=>$result_user_magento[0]['customer_id'], 'products'=>$decodedtext_product, 'address'=>$decodedtext_address, 'is_create_quote'=>1, 'shipmethod'=>'freeshipping_freeshipping', 'paymentmethod'=>'cashondelivery', 'couponCode'=>'null','transactionid' =>$encoded_trans_id, 'order_status' =>$value_uc_order['order_status'], 'trans_gross'=>$encoded_trans_mc_gross)
					);
					curl_setopt_array($ch, $curlConfig);
					$result = curl_exec($ch);				
					 $index = json_decode($result , true);
